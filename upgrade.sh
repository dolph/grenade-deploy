...
if [ ! -f ~/.rack/config ]; then
    echo "Configuring the rack client (interactive)..."
    ./rack configure
fi

echo "Deleting existing devstack-grenade server (if one exists)..."
./rack servers instance delete --name="devstack-grenade" || true

echo "Provisioning server..."
./rack servers instance create \
    --name="devstack-grenade" \
...
while true; do
    IP=`./rack servers instance list --name="devstack-grenade" --fields=publicipv4 --status=ACTIVE | sed -n 2p`
